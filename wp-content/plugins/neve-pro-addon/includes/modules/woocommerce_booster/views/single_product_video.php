...
class Single_Product_Video extends Base_View {
	...
	/**
	 * Check if video has rendered.
	 *
	 * @var bool
	 */
	private static $has_rendered_video = false;
	...
	public function replace_featured_image( $html, $thumbnail_id ) {
		...
		if ( (int) $thumbnail_id !== (int) $post_thumbnail_id ) {
			return $html;
		}
	...
	private function get_vimeo_thumbnail( $url ) {
		$id      = substr( $url, strrpos( $url, '/' ) + 1 );
		$content = function_exists( 'wpcom_vip_file_get_contents' ) ? wpcom_vip_file_get_contents( "https://vimeo.com/api/v2/video/$id.php" ) : file_get_contents( "https://vimeo.com/api/v2/video/$id.php" ); //phpcs:ignore WordPressVIPMinimum.Performance.FetchingRemoteData.FileGetContentsRemoteFile
		$api     = unserialize( $content ); //phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.serialize_unserialize
