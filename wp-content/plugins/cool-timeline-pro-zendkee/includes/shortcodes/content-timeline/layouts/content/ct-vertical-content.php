...
$ctl_v_html .= '<!-- .timeline-post-start-->';
  $ctl_v_html .= '<div  data-alternate="'.$i.'" id="post-'.esc_attr($post->ID).'" class="'.implode(" ",$p_cls).'"><div data-aos="'. $ctl_animation.'" class="timeline-meta">';
    ...
    if($layout!="compact" ){
     if($active_design=="design-6") {
        ...
        if ($disable_months == "no") {
          if ($layout!="compact") {
              $ctl_v_html .= '<div data-aos="'. $ctl_animation.'" class="meta-details">' . $posted_date . '</div>';
              }
            }
      }      
   }   
      ...
      $ctl_v_html .= '</div>';
       ...
       if($layout=="compact" && $active_design=="design-6"){
          if (isset($attribute['icons']) && $attribute['icons'] == "YES") {
         $ctl_v_html .= '<div data-aos="'. $ctl_animation.'" class="timeline-icon icon-larger iconbg-turqoise icon-color-white main-icon-' .esc_attr($design). '">
            <div class="icon-placeholder">' . $icon . '</div>
            <div class="timeline-bar"></div></div>';

            }else {
            $ctl_v_html .= '<div data-aos="'. $ctl_animation.'" class="timeline-icon icon-dot-full main-dot-' .esc_attr($design) . '">
            <div class="timeline-bar"></div>
            </div>';
            }
        }
    ...
    $ctl_v_html .= '<div data-aos="'. $ctl_animation.'"  id="' . esc_attr($row). '" class="timeline-content  clearfix ' .esc_attr($even_odd) . '  ' . esc_attr($container_cls) . '  ht-content-'.esc_attr($design).'">';

     if(in_array($active_design,array("design-2","default","design-4","design-5","design-6"))){
         if($active_design=="design-6"){
            $ctl_v_html .= '<div  data-aos="'. $ctl_animation.'" class="story-date clt-meta-date">' . $posted_date . '</div>';
           }else{
